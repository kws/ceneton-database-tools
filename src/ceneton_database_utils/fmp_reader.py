...
def read_fmp_xml(
    source: Any, header_strategy: HeaderStrategy = default_header_strategy
    ...
    for event, elem in ET.iterparse(
        ...
        # We are now in the results section - we do this first here because
        # this is the majority of the file so we reduce the number of "checks"
        # we need to do
        ...
        # In header mode we handle either the <DATABASE> element giving overall info,
        # or <FIELD> elements with field details
            ...
            if tag_name == "DATABASE":
                ...
                field_list.append(
                    FMPField(
                        name=field_name,
                        type=field_type,
                        empty_ok=field_empty_ok,
                        max_repeat=field_max_repeat,
                    )
                )
        ...
        if (
            ...
            if resultset_record_count != database_record_count:
                logger.warning(
                    f"Number of records in header ({database_record_count}) does not "
                    f"match number of records in result set ({resultset_record_count}). "  # noqa: E501
                    "We are looking at a subset of the database."
                )

            # We can now create the data enum
            cleaned_names = header_strategy(field_list)
            assert len(set(cleaned_names)) == len(field_list), (
                f"There are duplicate 'cleaned' names in the field list: {cleaned_names}. "  # noqa: E501
                "You will have to provide your own header_strategy to avoid this."
            )
            ...
            yield FMPSchema(
                ...
                record_count=resultset_record_count,
            )
